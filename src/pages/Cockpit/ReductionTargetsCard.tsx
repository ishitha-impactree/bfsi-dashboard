...
const ReductionTargetsCard = () => {
  ...
  const getStatusBg = (status: Target['status']): string => {
    switch (status) {
      case 'ahead': return 'bg-success/10';
      case 'on-track': return 'bg-primary/10';
      case 'at-risk': return 'bg-warning/10';
      case 'behind': return 'bg-error/10';
          ...
          <div key={target?.id} className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <div className={`w-8 h-8 rounded-full ${getStatusBg(target?.status)} flex items-center justify-center`}>
                  <Icon
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Target: {calculateReduction(target?.current, target?.target)}% reduction by {formatDate(target?.deadline)}
                  </p>
                </div>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-xs text-muted-foreground">
                <span>Current: {target?.current?.toLocaleString()} tCO₂e</span>
                <span>Target: {target?.target?.toLocaleString()} tCO₂e</span>
              </div>
