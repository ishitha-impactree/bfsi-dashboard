      ...
      <div className="p-4 border-b border-border">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Portfolio transactions and performance metrics</p>
          </div>
            ...
            <tr>
              ...
              <th className="text-right p-3 w-[120px]">
                <button
                  ...
                  className="flex items-center space-x-1 text-sm font-medium text-foreground hover:text-primary transition-colors duration-150 ml-auto"
                >
          ...
          <tbody>
            {sortedData.map((sector, index) => (
              ...
              >
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex items-center space-x-2">
                      <div className={`w-3 h-3 rounded-full ${
                        ...
                        index === 3 ? 'bg-purple-500' : 'bg-cyan-500'
                      }`} />
                ...
                <td className="p-3 text-right">
                  <span className="text-sm text-foreground">{sector.percentage}%</span>
                </td>
                
                <td className="p-3 text-right">
                  <div className={`flex items-center justify-end space-x-1 ${getChangeColor(sector.change)}`}>
                    <Icon name={getChangeIcon(sector.change)} size={12} />
                    <span className="text-sm font-medium">{sector.change}%</span>
                  </div>
                </td>
                
                <td className="p-3 text-right">
                  <span className="text-sm text-foreground">{sector.companies}</span>
                </td>
                
                <td className="p-3 text-right">
                  <span className="text-sm text-foreground">{sector.intensity}</span>
                </td>
        ...
        <div className="flex items-center justify-between text-sm">
          <div className="flex items-center space-x-4">
            <span className="text-muted-foreground">
              {selectedSectors.length > 0 ? `${selectedSectors.length} sectors selected` : 'No sectors selected'}
            </span>
          ...
          <div className="text-muted-foreground">
            Total: {sectorData.reduce((sum, sector) => sum + sector.emissions, 0).toLocaleString()} tCO₂e
          </div>
        </div>
      </div>
    </div>
