...
const ScatterPlotAnalysis: React.FC = () => {
  ...
  const currentMetric: MetricOption | undefined = metricOptions.find(m => m.value === selectedMetric);

  const averageEmissions: number = useMemo(() => {
    return scatterData.reduce((sum: number, item: ScatterDataItem) => sum + item.emissions, 0) / scatterData.length;
  }, [scatterData]);

  const averageMetric: number = useMemo(() => {
    return scatterData.reduce((sum: number, item: ScatterDataItem) => sum + item[selectedMetric], 0) / scatterData.length;
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-6 shadow-elevation-1">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 20, right: 20, bottom: 20, left: 20 }}>
            ...
            <XAxis 
              ...
              label={{ value: 'Emissions Intensity (tCO2e/$M)', position: 'insideBottom', offset: -10 }}
            />
            <YAxis 
              type="number" 
              ...
              tickLine={false}
              className="text-xs fill-muted-foreground"
              label={{ value: currentMetric?.yLabel, angle: -90, position: 'insideLeft' }}
            />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-4 border-t border-border">
        <div className="text-center">
          <div className="text-lg font-semibold text-foreground">
            {averageEmissions.toFixed(1)}
          </div>
          <div className="text-sm text-muted-foreground">Avg Emissions</div>
        </div>
        
        <div className="text-center">
          <div className="text-lg font-semibold text-foreground">
            {selectedMetric === 'returns' ? `${averageMetric.toFixed(1)}%` :
             selectedMetric === 'marketCap' ? `$${(averageMetric / 1000).toFixed(1)}T` :
             averageMetric.toFixed(0)}
          </div>
          <div className="text-sm text-muted-foreground">
            Avg {currentMetric?.label.split(' ')?.[0]}
          </div>
        </div>
        
        <div className="text-center">
          <div className="text-lg font-semibold text-foreground">-0.65</div>
      ...
      <div className="flex items-center justify-between mt-4 pt-4 border-t border-border">
        ...
        <Button variant="ghost" size="sm" iconName="Maximize2" iconSize={16}>
          Full Screen
        </Button>
      </div>
    </div>
